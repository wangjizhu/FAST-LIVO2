...
void loadVoxelConfig(rclcpp::Node *node, VoxelMapConfig &voxel_config)
{
  ...
  voxel_config.sigma_num_ = node->declare_parameter<double>("lio.sigma_num", 3);
  ...
  // ROS2 only supports int64 arrays as parameters; convert to the int vector the code expects.
  {
  ...
  voxel_config.sliding_thresh = node->declare_parameter<double>("local_map.sliding_thresh", 8);
}
                                     ...
                                     const float alpha, const Eigen::Vector3d rgb)
{
  ...
  plane.header.stamp = rclcpp::Time(0, 0);
  plane.ns = plane_ns;
