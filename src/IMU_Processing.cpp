...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  if (!imu_time_init)
  {
    // imu_time = toSec(v_imu.front()->header.stamp) - first_lidar_time;
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      // cout<<setw(20)<<"offset_t: "<<offs_t<<"toSec(tail->header.stamp):
      // "<<toSec(tail->header.stamp)<<endl; printf("[ LIO Propagation ]
      // offs_t: %lf \n", offs_t);
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      imu_need_init = false;
      RCLCPP_INFO(rclcpp::get_logger("ImuProcess"), "IMU Initials: Gravity: %.4f %.4f %.4f %.4f; acc covarience: "
               ...
               cov_gyr[2]);
      RCLCPP_INFO(rclcpp::get_logger("ImuProcess"), "IMU Initials: ba covarience: %.8f %.8f %.8f; bg covarience: "
               "%.8f %.8f %.8f",
