...
namespace
{
// ROS1 ros::Time helpers re-expressed for ROS2 builtin_interfaces::msg::Time.
inline double toSec(const builtin_interfaces::msg::Time &t) { return rclcpp::Time(t).seconds(); }
inline builtin_interfaces::msg::Time toRosTime(double t)
{
  builtin_interfaces::msg::Time stamp;
  int32_t sec = static_cast<int32_t>(std::floor(t));
  stamp.sec = sec;
  stamp.nanosec = static_cast<uint32_t>((t - static_cast<double>(sec)) * 1e9);
  return stamp;
}
} // namespace
...
void LIVMapper::readParameters()
{
  ...
  ros_driver_fix_en = this->declare_parameter<bool>("common.ros_driver_bug_fix", false);
  img_en = this->declare_parameter<int>("common.img_en", 1);
  lidar_en = this->declare_parameter<int>("common.lidar_en", 1);
  img_topic = this->declare_parameter<string>("common.img_topic", "/left_camera/image");

  normal_en = this->declare_parameter<bool>("vio.normal_en", true);
  inverse_composition_en = this->declare_parameter<bool>("vio.inverse_composition_en", false);
  max_iterations = this->declare_parameter<int>("vio.max_iterations", 5);
  IMG_POINT_COV = this->declare_parameter<double>("vio.img_point_cov", 100);
  raycast_en = this->declare_parameter<bool>("vio.raycast_en", false);
  exposure_estimate_en = this->declare_parameter<bool>("vio.exposure_estimate_en", true);
  inv_expo_cov = this->declare_parameter<double>("vio.inv_expo_cov", 0.2);
  grid_size = this->declare_parameter<int>("vio.grid_size", 5);
  grid_n_height = this->declare_parameter<int>("vio.grid_n_height", 17);
  patch_pyrimid_level = this->declare_parameter<int>("vio.patch_pyrimid_level", 3);
  patch_size = this->declare_parameter<int>("vio.patch_size", 8);
  outlier_threshold = this->declare_parameter<double>("vio.outlier_threshold", 1000);

  exposure_time_init = this->declare_parameter<double>("time_offset.exposure_time_init", 0.0);
  img_time_offset = this->declare_parameter<double>("time_offset.img_time_offset", 0.0);
  imu_time_offset = this->declare_parameter<double>("time_offset.imu_time_offset", 0.0);
  lidar_time_offset = this->declare_parameter<double>("time_offset.lidar_time_offset", 0.0);
  imu_prop_enable = this->declare_parameter<bool>("uav.imu_rate_odom", false);
  gravity_align_en = this->declare_parameter<bool>("uav.gravity_align_en", false);

  seq_name = this->declare_parameter<string>("evo.seq_name", "01");
  pose_output_en = this->declare_parameter<bool>("evo.pose_output_en", false);
  gyr_cov = this->declare_parameter<double>("imu.gyr_cov", 1.0);
  acc_cov = this->declare_parameter<double>("imu.acc_cov", 1.0);
  imu_int_frame = this->declare_parameter<int>("imu.imu_int_frame", 3);
  imu_en = this->declare_parameter<bool>("imu.imu_en", false);
  gravity_est_en = this->declare_parameter<bool>("imu.gravity_est_en", true);
  ba_bg_est_en = this->declare_parameter<bool>("imu.ba_bg_est_en", true);

  p_pre->blind = this->declare_parameter<double>("preprocess.blind", 0.01);
  filter_size_surf_min = this->declare_parameter<double>("preprocess.filter_size_surf", 0.5);
  hilti_en = this->declare_parameter<bool>("preprocess.hilti_en", false);
  p_pre->lidar_type = this->declare_parameter<int>("preprocess.lidar_type", AVIA);
  p_pre->N_SCANS = this->declare_parameter<int>("preprocess.scan_line", 6);
  p_pre->point_filter_num = this->declare_parameter<int>("preprocess.point_filter_num", 3);
  p_pre->feature_enabled = this->declare_parameter<bool>("preprocess.feature_extract_enabled", false);
  ...
  pcd_save_type = this->declare_parameter<int>("pcd_save.type", 0);
  img_save_en = this->declare_parameter<bool>("image_save.img_save_en", false);
  img_save_interval = this->declare_parameter<int>("image_save.interval", 1);

  colmap_output_en = this->declare_parameter<bool>("pcd_save.colmap_output_en", false);
  filter_size_pcd = this->declare_parameter<double>("pcd_save.filter_size_pcd", 0.5);
  extrinT = this->declare_parameter<vector<double>>("extrin_calib.extrinsic_T", vector<double>());
  extrinR = this->declare_parameter<vector<double>>("extrin_calib.extrinsic_R", vector<double>());
  cameraextrinT = this->declare_parameter<vector<double>>("extrin_calib.Pcl", vector<double>());
  cameraextrinR = this->declare_parameter<vector<double>>("extrin_calib.Rcl", vector<double>());
  plot_time = this->declare_parameter<double>("debug.plot_time", -10);
  frame_cnt = this->declare_parameter<int>("debug.frame_cnt", 6);

  blind_rgb_points = this->declare_parameter<double>("publish.blind_rgb_points", 0.01);
  pub_scan_num = this->declare_parameter<int>("publish.pub_scan_num", 1);
  pub_effect_point_en = this->declare_parameter<bool>("publish.pub_effect_point_en", false);
  dense_map_en = this->declare_parameter<bool>("publish.dense_map_en", false);
...
void LIVMapper::initializeComponents() 
{
  ...
  // --- Self-contained camera construction (replaces vikit_ros camera_loader, which
  //     relied on the ROS1 global parameter server). Camera intrinsics are read from
  //     this node's parameters, supplied by the camera_*.yaml file. ---
  {
    std::string cam_model = this->declare_parameter<std::string>("cam_model", "Pinhole");
    int cam_width = this->declare_parameter<int>("cam_width", 640);
    int cam_height = this->declare_parameter<int>("cam_height", 480);
    double scale = this->declare_parameter<double>("scale", 1.0);
    double cam_fx = this->declare_parameter<double>("cam_fx", 400.0);
    double cam_fy = this->declare_parameter<double>("cam_fy", 400.0);
    double cam_cx = this->declare_parameter<double>("cam_cx", cam_width / 2.0);
    double cam_cy = this->declare_parameter<double>("cam_cy", cam_height / 2.0);

    if (cam_model == "Pinhole")
    {
      double d0 = this->declare_parameter<double>("cam_d0", 0.0);
      double d1 = this->declare_parameter<double>("cam_d1", 0.0);
      double d2 = this->declare_parameter<double>("cam_d2", 0.0);
      double d3 = this->declare_parameter<double>("cam_d3", 0.0);
      vio_manager->cam = new vk::PinholeCamera(cam_width, cam_height, scale, cam_fx, cam_fy, cam_cx, cam_cy, d0, d1, d2, d3);
    }
    else if (cam_model == "EquidistantCamera")
    {
      double k1 = this->declare_parameter<double>("k1", 0.0);
      double k2 = this->declare_parameter<double>("k2", 0.0);
      double k3 = this->declare_parameter<double>("k3", 0.0);
      double k4 = this->declare_parameter<double>("k4", 0.0);
      vio_manager->cam = new vk::EquidistantCamera(cam_width, cam_height, scale, cam_fx, cam_fy, cam_cx, cam_cy, k1, k2, k3, k4);
    }
    else
    {
      throw std::runtime_error("Camera model '" + cam_model + "' is not supported by this ROS2 port (use 'Pinhole' or 'EquidistantCamera').");
    }
  }
...
{
  const rclcpp::QoS qos = rclcpp::QoS(rclcpp::KeepLast(200000));

  if (p_pre->lidar_type == AVIA)
  {
    sub_pcl = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
...
{    
  ...
  euler_cur = RotMtoEuler(_state.rot_end);
  {
    ...
    quat_tf.setRPY(euler_cur(0), euler_cur(1), euler_cur(2));
    geoQuat = tf2::toMsg(quat_tf);
  }
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  // RCLCPP_INFO(this->get_logger(), "get point cloud at time: %.6f", toSec(msg->header.stamp));
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  ...
  // if ((abs(toSec(msg->header.stamp) - last_timestamp_lidar) > 0.2 && last_timestamp_lidar > 0) || sync_jump_flag)
  // {
  //   RCLCPP_WARN(this->get_logger(), "lidar jumps %.3f\n", toSec(msg->header.stamp) - last_timestamp_lidar);
  //   sync_jump_flag = true;
  //   msg->header.stamp = toRosTime(last_timestamp_lidar + 0.1);
  // }
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  // RCLCPP_INFO(this->get_logger(), "get imu at time: %.6f", toSec(msg_in->header.stamp));
  sensor_msgs::msg::Imu::SharedPtr msg = std::make_shared<sensor_msgs::msg::Imu>(*msg_in);
  msg->header.stamp = toRosTime(toSec(msg->header.stamp) - imu_time_offset);
...
{
  if (!img_en) return;
  sensor_msgs::msg::Image::SharedPtr msg = std::make_shared<sensor_msgs::msg::Image>(*msg_in);
  // if ((abs(toSec(msg->header.stamp) - last_timestamp_img) > 0.2 && last_timestamp_img > 0) || sync_jump_flag)
  // {
  //   RCLCPP_WARN(this->get_logger(), "img jumps %.3f\n", toSec(msg->header.stamp) - last_timestamp_img);
  //   sync_jump_flag = true;
  //   msg->header.stamp = toRosTime(last_timestamp_img + 0.1);
  // }
